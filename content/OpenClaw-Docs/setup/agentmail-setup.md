...
AgentMail receives it
    ↓
Webhook fires to your OpenClaw instance (via Tailscale Funnel)
    ↓
...
1. **OpenClaw running** with Tailscale Funnel enabled (`gateway.tailscale.mode: "funnel"`)
...
## Step 4: Configure OpenClaw Webhook Endpoint
...
export default function(input: any) {
  // ⚠️ IMPORTANT: OpenClaw wraps the webhook body in input.payload
...
### OpenClaw transform input (what your function receives)
```json
{
...
⚠️ **Key insight:** OpenClaw wraps the incoming webhook body in `input.payload`, so access the message via `input.payload.message`.
