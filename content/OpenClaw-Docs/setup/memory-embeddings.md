...
OpenClaw uses **semantic search** to recall information from memory files (`MEMORY.md` and `memory/*.md`). This is powered by text embeddings — numerical representations of text that capture meaning.
...
Embedding settings live in OpenClaw's config. The default uses OpenAI with the API key from your auth profile.
